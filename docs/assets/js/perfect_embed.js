 ...
 * </div>
 * 
 * OPCIONAL (apenas SVG): data-color-scheme="light" renderiza o diagrama no
 * esquema claro. Use em exports do draw.io, cujas formas tem fill branco fixo
 * enquanto os rotulos usam light-dark() e ficariam brancos no tema escuro.
 *
 ...
 * </div>
 *
 * ALTURA: e derivada do formato do artefato (limitada a 85vh), para que ele
 * preencha o quadro sem faixas vazias. Para fixar, use style="height: XXXpx".
 * =============================================================================
 */
...
const CANVAS = Object.freeze({
  // Teto para a altura derivada do formato do artefato, para que um
  // diagrama alto nao ultrapasse a altura da tela.
  MAX_HEIGHT: "75vh",
});
...
/**
 * Faz a área de desenho assumir o formato do artefato (SVG ou imagem), para
 * que ele ocupe todo o quadro em vez de deixar faixas vazias nas laterais.
 * Só se aplica quando o container não define uma altura fixa.
 */
function fitCanvasToAspectRatio(rootElement, canvas, width, height) {
  if (!canvas || !width || !height) return;
  if (rootElement.style.height && rootElement.style.height !== "auto") return;

  canvas.style.aspectRatio = `${width} / ${height}`;
  canvas.style.flex = "0 0 auto";
  canvas.style.maxHeight = CANVAS.MAX_HEIGHT;
}

/**
 * Substitui `light-dark(claro, escuro)` pelo valor claro, preservando o resto
 * da string. Aceita argumentos com parenteses aninhados, como
...
function resolveLightDark(value) {
  ...
  while (index < value.length) {
    const start = value.indexOf(TOKEN, index);
    ...
    let comma = -1;
    let end = -1;
    for (let i = start + TOKEN.length - 1; i < value.length; i++) {
      const char = value[i];
      if (char === "(") depth++;
    ...
    // Expressao malformada: preserva o texto original.
    if (end === -1 || comma === -1) return result + value.slice(start);

    result += resolveLightDark(value.slice(start + TOKEN.length, comma)).trim();
    index = end + 1;
  }
...
/**
 * Fixa o esquema claro em um diagrama do draw.io. O export usa `light-dark()`
 * na cor do texto e no fundo do rotulo, resolvido a partir do esquema de cores
 * em uso: em aparelhos no modo escuro (comum no mobile) o rotulo e pintado em
 * branco sobre o diagrama claro, e o WebKit chega a reavaliar o esquema ao
 * rasterizar de novo em zoom alto, embranquecendo o texto no meio da leitura.
 * Resolver a funcao no DOM torna o resultado igual em qualquer navegador,
 * inclusive nos que ainda nao suportam `light-dark()`.
 */
function forceLightColorScheme(svg) {
  svg.style.colorScheme = "light";

  svg.querySelectorAll("style").forEach((style) => {
    if (style.textContent.includes("light-dark(")) {
      style.textContent = resolveLightDark(style.textContent);
    }
  });

  [svg, ...svg.querySelectorAll("*")].forEach((element) => {
    Array.from(element.attributes).forEach((attr) => {
      // `content` guarda o mxfile original; o botao de download serializa
      // o DOM, entao o diagrama baixado continua editavel no draw.io.
      if (attr.name === "content") return;
      if (attr.value.includes("light-dark(")) {
        element.setAttribute(attr.name, resolveLightDark(attr.value));
      }
    });
  });
}
...
class ImagePanZoom {
  ...
  /**
   * Tamanho que a imagem realmente ocupa em 100%. Como o elemento usa
   * `object-fit: contain` dentro do quadro, o zoom padrao ja mostra a imagem
   * inteira e o conteudo pode ser menor que a caixa (faixas nas laterais).
   */
  _fittedSize() {
  ...
  _clampedPan(panX, panY) {
    const fitted = this._fittedSize();
    const scaledWidth = fitted.width * (this.currentZoom / 100);
    const scaledHeight = fitted.height * (this.currentZoom / 100);
    const { width, height } = this.canvas.getBoundingClientRect();
  ...
  /**
   * Quando o quadro muda de tamanho (layout responsivo, tela cheia), o
   * enquadramento e recalculado; se o usuario ainda nao alterou o zoom, a
   * imagem volta a ser exibida por inteiro.
   */
  _observeCanvasResize() {
    if (typeof ResizeObserver === "undefined") return;

    const observer = new ResizeObserver(() => {
      if (this.currentZoom === ZOOM.DEFAULT) {
        this.panX = 0;
        this.panY = 0;
      }
      this._scheduleRender();
  ...
  _updateCoordinates(clientX, clientY) {
    ...
    const fitted = this._fittedSize();
    const scale = (fitted.width / this.img.naturalWidth) * (this.currentZoom / 100);
    const imgX = this.img.naturalWidth / 2 + (clientX - rect.left - centerX) / scale;
    const imgY = this.img.naturalHeight / 2 + (clientY - rect.top - centerY) / scale;
...
const EMBED_STYLE_TEMPLATE = `
  <style>
    ...
    .embed-container:fullscreen .embed-canvas {
...
class SvgEmbed {
  async create(rootElement) {
    ...
    try {
      ...
      if (rootElement.dataset.colorScheme === "light") forceLightColorScheme(svg);
      ...
      fitCanvasToAspectRatio(rootElement, document.getElementById(`canvas-area-${id}`), viewBox.width, viewBox.height);
  ...
  _fitSvgToCanvas(svg, viewBox) {
    ...
    // "slice" faz o diagrama cobrir o quadro quando a altura e limitada por
    // CANVAS.MAX_HEIGHT; sem o limite, o quadro ja tem o formato do diagrama.
...
class ImageEmbed {
  async create(rootElement) {
    ...
    fitCanvasToAspectRatio(rootElement, canvas, img.naturalWidth, img.naturalHeight);
  ...
  _createImageElement(id, src) {
    ...
    Object.assign(img.style, {
      // Zoom padrao (100%) mostra a imagem inteira: ela e reduzida para
      // caber no quadro sem cortes. As regras `max*` neutralizam o CSS do
      // tema, que limitaria a imagem a largura do conteudo.
      width: "100%",
      height: "100%",
      maxWidth: "100%",
      maxHeight: "100%",
...
(function () {
  ...
  /* ──────────────────────────────────────────────────────────
     4. DATE COLUMNS
     Identifica colunas de data (pelo cabeçalho ou formato DD/MM/AAAA)
     e versão, marcando com data-col="date" / "version" para proibir
     quebra de linha.
  ────────────────────────────────────────────────────────── */
  function initDateColumns() {
    ...
    tables.forEach(function (table) {
      if (table.dataset.dateColumnsBound) return;
      table.dataset.dateColumnsBound = 'true';

      var dateColIndices = [];
      var versionColIndices = [];
      var headers = table.querySelectorAll('th');
      ...
      headers.forEach(function (th, i) {
        ...
        if (text.includes('data') || text.includes('date')) {
          ...
          th.setAttribute('data-col', 'date');
        } else if (text === 'versão' || text === 'versao' || text === 'version') {
          versionColIndices.push(i);
          th.setAttribute('data-col', 'version');
        }
      });

      var dateRegex = /^\s*\d{1,2}\/\d{1,2}\/\d{2,4}\s*$/;
      var rows = table.querySelectorAll('tbody tr');

      rows.forEach(function (row) {
        dateColIndices.forEach(function (idx) {
          if (row.cells[idx]) {
            row.cells[idx].setAttribute('data-col', 'date');
          }
        });

        versionColIndices.forEach(function (idx) {
          if (row.cells[idx]) {
            row.cells[idx].setAttribute('data-col', 'version');
          }
        });

        Array.from(row.cells).forEach(function (cell) {
          if (dateRegex.test(cell.textContent)) {
            cell.setAttribute('data-col', 'date');
          }
        });
      });
    });
