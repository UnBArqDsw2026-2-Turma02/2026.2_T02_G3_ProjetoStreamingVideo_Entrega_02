...
Representa um usuário registrado e autenticado na plataforma, gerenciando credenciais e configurações de conta locais.
...
Representa o perfil público de um usuário (ou de um ator federado remotamente), servindo como a identidade visível na plataforma e dona dos canais.
...
Um canal que atua como um agrupador para publicar e organizar os vídeos de uma conta.
...
A entidade central que representa um vídeo na plataforma, agregando todos os metadados e metadados de visualização do vídeo.
...
Representa os arquivos físicos de mídia do vídeo, descrevendo atributos como resolução, tamanho e formato. Pode representar um arquivo independente (WebVideo) ou fragmentos de uma playlist (HLS).
...
Representa uma playlist de streaming adaptativo, geralmente do tipo HLS (HTTP Live Streaming), contendo e organizando múltiplos arquivos segmentados.
...
Utilizada para rastrear e gerenciar o estado das tarefas assíncronas em segundo plano relacionadas ao vídeo (como uploads e transcodificação).
...
Responsável por manter o registro do histórico de visualização do usuário logado, possibilitando que ele retome o vídeo de onde parou.
